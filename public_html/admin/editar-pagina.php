<?php
// admin/editar-pagina.php (VERSÃO COM UPLOAD DE IMAGEM)
...
$mensagem = '';
$pagina = null;
$uploads_dir = '../uploads/paginas/'; // (Esta é para a foto de destaque)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['acao']) && $_POST['acao'] == 'editar') {
    ...
    $conteudo = $_POST['conteudo']; // TinyMCE envia HTML
    $caminho_imagem_atual = $_POST['imagem_atual'];

    // --- Lógica de Upload da Foto de Destaque (página 'sobre') ---
    if (isset($_FILES['imagem_destaque']) && $_FILES['imagem_destaque']['error'] == UPLOAD_ERR_OK) {
        ...
        if ($check !== false) {
            ...
            } else { $mensagem = '<div class="alerta erro">Erro ao mover o arquivo de upload.</div>'; }
        } else { $mensagem = '<div class="alerta erro">O arquivo enviado não é uma imagem válida.</div>'; }
    }
    ...
    if (empty($mensagem)) {
        try {
            ...
            exit;
        } catch (PDOException $e) { $mensagem = '<div class="alerta erro">Erro ao atualizar a página.</div>'; }
    }
}
...
$stmt->execute([$id]);
$pagina = $stmt->fetch();
if (!$pagina) { header("Location: gerenciar-paginas.php"); exit; }
...
<p><a href="gerenciar-paginas.php">&larr; Voltar para a lista de páginas</a></p>
...
<div class="admin-form">
    <form action="editar-pagina.php?id=<?php echo $pagina['id']; ?>" method="POST" enctype="multipart/form-data">
        ...
        <?php if ($pagina['slug'] == 'sobre'): // Só mostra o upload da foto de destaque para a pág 'sobre' ?>
            <div class="form-group">
        ...
        <div class="form-group">
            ...
            <textarea id="conteudo" name="conteudo" rows="20"><?php echo htmlspecialchars($pagina['conteudo']); ?></textarea>
        </div>
...
<script>
    tinymce.init({
        selector: '#conteudo',
        plugins: 'lists link wordcount image', // ADICIONADO 'image'
        toolbar: 'undo redo | blocks | bold italic | alignleft aligncenter alignright | bullist numlist | link image', // ADICIONADO 'image'
        menubar: false,
        
        // --- A MÁGICA DO UPLOAD (NOVO) ---
        images_upload_url: 'tinymce-upload.php', // Aponta para o nosso script
